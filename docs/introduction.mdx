...
Hydra is a real-time object detection and tracking payload for uncrewed vehicles. Drones, boats, rovers -- if it runs [ArduPilot](https://ardupilot.org/) and has a camera, Hydra gives it eyes.

It runs on an NVIDIA Jetson (or any Linux box with a camera and a MAVLink radio) and hooks into your existing flight stack with zero firmware changes.
...
<Card title="See things" icon="eye">
  YOLOv8 object detection running in real-time on-device via CUDA / TensorRT. Configurable confidence thresholds and class filters.
</Card>

<Card title="Remember them" icon="fingerprint">
  ByteTrack keeps persistent IDs across frames. Object #5 stays object #5 through occlusions and re-entries.
</Card>

<Card title="Tell you about them" icon="satellite-dish">
  MAVLink STATUSTEXT alerts sent to Mission Planner or QGroundControl with GPS coordinates and class labels.
</Card>

<Card title="Keep them in frame" icon="crosshairs">
  Lock onto a tracked target and the vehicle yaws to keep it centered in the camera field of view.
</Card>

<Card title="Go to them" icon="location-arrow">
  Strike mode estimates target GPS from vehicle heading and camera offset, then navigates in GUIDED mode.
</Card>

<Card title="Hunt RF sources" icon="wifi">
  Locate WiFi APs or SDR signals using Kismet RSSI and gradient ascent navigation. Supports lawnmower and spiral search patterns.
</Card>

<Card title="Auto-engage targets" icon="bolt">
  Autonomous strike controller with geofencing (circle or polygon), class whitelists, confidence gates, and cooldown timers.
</Card>

<Card title="Review missions" icon="map">
  Post-flight web map with detection markers, track trails, confidence filters, and class filtering over OpenStreetMap.
</Card>

<Card title="Stream to browser" icon="display">
  Live annotated MJPEG video stream with bounding boxes, track IDs, and target lock overlays on the operator dashboard.
</Card>

<Card title="Show on goggles" icon="vr-cardboard">
  Detection telemetry overlaid on your FPV feed through the flight controller's OSD chip. No extra hardware, no added latency.
</Card>
...
<Card title="Quickstart" icon="rocket" href="/quickstart">
  Install Hydra and get the dashboard running in under 5 minutes.
</Card>
...
<Card title="Configuration reference" icon="gear" href="/reference/configuration">
  Every config.ini knob documented.
</Card>
