...
This page provides the same map visualization but with interactive controls:

- Browse and load available log files via the [`GET /api/review/logs`](/api-reference) endpoint
- Switch between log files without reloading the page
...
Both the CLI export and the web review page share the same visualization features:
...
<CardGroup cols={2}>
  ...
  <Card title="Confidence filters" icon="sliders">
    Filter detections by minimum confidence score to focus on high-quality detections and hide uncertain ones.
  </Card>
...
- Track ID (ByteTrack persistent ID)
- Vehicle position and heading at time of detection
