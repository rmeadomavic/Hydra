...
3. **HOMING** — RSSI above threshold, following the gradient ascent toward the source
...
When an RF hunt starts, the vehicle flies a search pattern centered on its current position (or a configured start point). Two patterns are available:
...
The [web dashboard](/features/dashboard) provides a full RF hunt interface:

- Configure hunt parameters (mode, target, thresholds)
- Start and stop hunts
- Monitor RSSI readings in real time
- View current hunt state (IDLE / SEARCHING / HOMING / CONVERGED)
