...
**Date:** 2026-05-25 (post-merge-window review)
**Commit:** c483d22 — `fix(#158): correct firmware detection — use HEARTBEAT.type not autopilot_id`

Post-PR review caught a bug all three adversarial rounds missed: `detect_fc` was deriving `firmware` from `AUTOPILOT_VERSION.autopilot_id` with a magic-number map `{3: ArduCopter, 4: ArduPlane, 12: ArduRover}`. Two errors in one:

1. **`AUTOPILOT_VERSION` carries no `autopilot_id` field.** That field name lives on `HEARTBEAT`, where it is the MAV_AUTOPILOT enum (ArduPilot vs. PX4 vs. generic), not a vehicle-variant disambiguator.
2. **Even if the field existed, MAV_AUTOPILOT=3 means ARDUPILOTMEGA**, which all ArduPilot variants report — Copter, Plane, Rover, and Sub all set autopilot=3. The variant disambiguator lives in `HEARTBEAT.type` (MAV_TYPE: QUADROTOR, FIXED_WING, GROUND_ROVER, SURFACE_BOAT, ...).

Tests passed on the original mocks because the mocks returned whatever the test scripted — i.e. the tests scripted a fake `autopilot_id` and asserted on the lookup result. On real hardware, `getattr(autopilot_version_msg, "autopilot_id", 0)` would return `0` (default) for every platform, mapping every non-Copter SORCC platform (ZOHD plane, SCX6 UGV, Bonzi USV) to `firmware="unknown"`.

**Why the rounds missed it:** All three rounds audited the wizard at the protocol-level (race windows, multi-writer, partial failure) and treated the spec's `autopilot_id` field as authoritative. None re-derived the field from the MAVLink message catalog. Pattern-match round (R1) trusted the spec; counter-critique (R2) sharpened existing findings rather than re-questioning premises; orthogonal sweep (R3) found multi-writer framing but stayed inside "the wizard is the only writer assumed correct." Lesson for future adversarial rounds: cross-reference field names against the pymavlink message dictionary before trusting them, especially for fields whose names match by accident (`autopilot_id` reads like a vehicle ID; it's a protocol-family enum).

**Fix shape:** `detect_fc` now waits for HEARTBEAT (1s timeout), reads `type` and `autopilot`, gates firmware on `autopilot == MAV_AUTOPILOT_ARDUPILOTMEGA (3)`, then looks up `HEARTBEAT.type` in a MAV_TYPE → firmware map covering 17 MAV_TYPE values across Copter (QUAD/COAXIAL/HELI/HEX/OCTO/TRI), Plane (FIXED_WING + 7 VTOL variants), Rover (GROUND_ROVER for UGV + SURFACE_BOAT for USV — same binary, different vehicle), and Sub. `AUTOPILOT_VERSION.flight_sw_version` is still read for the version string. Return shape preserved (`firmware`, `version`, `frame_type`, `autopilot_id`) for `server.py` back-compat; HEARTBEAT timeout returns `None` instead of `0` to avoid false zero-defaults.

**Tests:** Replaced 4 detect_fc cases with 9 that mock the HEARTBEAT-then-AUTOPILOT_VERSION sequence, covering: ArduCopter quad, non-ArduPilot autopilot rejection, ArduRover UGV vs USV both mapping to "ArduRover", ArduPlane fixed-wing + VTOL, unknown MAV_TYPE → unknown, HEARTBEAT timeout, AUTOPILOT_VERSION timeout preserving firmware. Total 24/24 pass.

Original 3 rounds and consolidated risk register above remain valid — R3-1 (multi-writer post_value) is still the load-bearing R3 finding and still a PR-B gate.
