...
<Note>
Control endpoints (`POST` routes for target lock, strike, vehicle commands, pipeline control, and RF hunt) require bearer token authentication. Include your token in the `Authorization` header: `Authorization: Bearer <token>`.
</Note>
