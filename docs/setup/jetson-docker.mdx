...
[Jetson flash](/setup/jetson-flash) first.
...
<Tip>
YOLO (yolov8n) downloads its model automatically on first run
...
<Warning>
Always use `--runtime nvidia` -- without it, OpenCV will crash with
`libwayland-cursor.so.0: file too short` because the container needs the
host's NVIDIA libraries mounted in.
</Warning>
