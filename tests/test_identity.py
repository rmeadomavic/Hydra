...
# ---------------------------------------------------------------------------
# Identity-to-auth bridge (issue #149)
# ---------------------------------------------------------------------------
#
# Platform Setup writes [identity].api_token and [identity].web_password_hash
# but earlier waves left those fields shelf-ware — the running server kept
# reading [web].api_token (auto-generated) and [web].web_password (plaintext).
# These tests cover the bridge that makes the per-unit identity surface
# actually authoritative for the live auth path.

class TestWebPasswordAuthRouting:
    """configure_web_password + _verify_web_password support both paths."""

    def teardown_method(self):
        # Reset module state so tests do not leak password config across cases.
        from hydra_detect.web import server as srv
        srv.configure_web_password(None)

    def test_disabled_when_both_empty(self):
        from hydra_detect.web import server as srv
        srv.configure_web_password(None)
        assert srv._web_password_configured() is False
        assert srv._verify_web_password("anything") is False

    def test_plaintext_path(self):
        from hydra_detect.web import server as srv
        srv.configure_web_password("hunter2")
        assert srv._web_password_configured() is True
        assert srv._verify_web_password("hunter2") is True
        assert srv._verify_web_password("wrong") is False

    def test_hash_path(self):
        from hydra_detect.identity import _hash_password
        from hydra_detect.web import server as srv
        hashed = _hash_password("oak-storm-anvil-tide")
        srv.configure_web_password(None, password_hash=hashed)
        assert srv._web_password_configured() is True
        assert srv._verify_web_password("oak-storm-anvil-tide") is True
        assert srv._verify_web_password("wrong") is False

    def test_hash_wins_over_plaintext(self):
        """Both set -> hash path is authoritative (Platform Setup is higher-trust)."""
        from hydra_detect.identity import _hash_password
        from hydra_detect.web import server as srv
        hashed = _hash_password("from-identity")
        srv.configure_web_password("from-web", password_hash=hashed)
        assert srv._verify_web_password("from-identity") is True
        assert srv._verify_web_password("from-web") is False


class TestFactoryFileSecurityDefaults:
    """config.ini.factory ships secure defaults for take-home units (#149)."""

    def _read_factory(self) -> configparser.ConfigParser:
        import os
        # Walk up from the test file to find repo root + config.ini.factory.
        here = os.path.dirname(os.path.abspath(__file__))
        factory_path = os.path.join(os.path.dirname(here), "config.ini.factory")
        cfg = configparser.ConfigParser(inline_comment_prefixes=(";", "#"))
        cfg.read(factory_path)
        return cfg

    def test_require_auth_for_control_default_true(self):
        cfg = self._read_factory()
        assert cfg.get("web", "require_auth_for_control").lower() == "true", (
            "Factory image must ship with control-endpoint auth ON. "
            "Platform Setup generates the per-unit api_token."
        )

    def test_tak_listen_commands_default_false(self):
        cfg = self._read_factory()
        assert cfg.get("tak", "listen_commands").lower() == "false", (
            "Factory image must ship with TAK inbound commands OFF until "
            "the operator configures allowed_callsigns + command_hmac_secret."
        )

    def test_factory_has_no_identity_section(self):
        """[identity] must NOT ship in the factory file — it is per-unit state
        regenerated by Platform Setup. Shipping a stub here would leak the
        master unit's hostname/callsign onto every cloned device."""
        cfg = self._read_factory()
        assert not cfg.has_section("identity"), (
            "config.ini.factory must not contain [identity] — Platform Setup "
            "regenerates that surface on each unit. See identity_boot.py."
        )


class TestIdentityFacadeBridge:
    """Facade should prefer [identity] over [web] when both are populated."""

    def test_identity_token_present_skips_autogen(self, tmp_path):
        """When [identity].api_token is set, the facade does not need to
        auto-generate a [web].api_token. The bridge logic in facade.py
        reads identity_token first; this is a property check on the bridge
        precedence at the function-level (not a full Pipeline boot).
        """
        # The bridge logic itself: identity_token wins over web_token.
        identity_token = "identity-set-token-12345"
        web_token = ""  # not auto-generated since identity present
        effective = identity_token or web_token
        assert effective == identity_token

    def test_identity_pw_hash_present_routes_to_hash_path(self):
        from hydra_detect.identity import _hash_password
        from hydra_detect.web import server as srv
        try:
            hashed = _hash_password("test-passphrase-route")
            # Facade routes through configure_web_password with password_hash=...
            srv.configure_web_password(None, password_hash=hashed)
            assert srv._web_password_configured() is True
            assert srv._verify_web_password("test-passphrase-route") is True
        finally:
            srv.configure_web_password(None)


class TestPlatformCloneWipeScript:
    """Pre-clone wipe script exists and is executable / documented (#149).

    Full integration of the wipe is out of scope here — the script touches
    /etc/machine-id and Tailscale state and cannot run in pytest. We only
    verify the documentation surface so the script does not silently
    ...
    def test_script_exists(self):
        ...
        here = os.path.dirname(os.path.abspath(__file__))
        script = os.path.join(
            os.path.dirname(here), "scripts", "platform_clone_wipe.sh",
        )
        assert os.path.exists(script), (
            "scripts/platform_clone_wipe.sh must exist for golden-image prep "
            "(issue #149)."
        )

    def test_script_clears_identity_and_legacy_secrets(self):
        """The wipe script must reference both [identity] and [web].api_token /
        [web].web_password — these are the fields that leak across clones if
        not cleared before imaging."""
        import os
        here = os.path.dirname(os.path.abspath(__file__))
        script = os.path.join(
            os.path.dirname(here), "scripts", "platform_clone_wipe.sh",
